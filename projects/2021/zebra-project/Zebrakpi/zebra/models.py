...
class GroupCategoryMatrix(models.Model):
    ...
    def __str__(self):
        return f"[ {self.activity_group.activity_name} ] {self.category_name.cat_name}"
        #{self.group_category_name.groucattypes_name}
...
class User(models.Model):
    role = models.ForeignKey(Role, on_delete=models.CASCADE, related_name='roles', default='cc_controller')
...
class SiteLanguageCountryMatrix(models.Model):
    ...
    def __str__(self):
        return "%s %s %s" % (self.site_master, self.site_lang_matrix_name, self.country_master.country_name)
...
class UserSiteLanguageCountryMatrix(models.Model):
    ...
    def __str__(self):
        return "%s %s %s" % (self.role_relation.role_name, self.user_relation.username, self.site_lang_coun_matrix.site_master.site_name)
...
class KpiDefinition(models.Model):
    ...
    def __str__(self):
        return "%s %s %s" % (self.kpi_group_def.group_category_name)
...
class KpiData(models.Model):
    kpi_data_name = models.ForeignKey(
        SiteLanguageCountryMatrix, on_delete=models.CASCADE, related_name="kpi_data_name")
    status_name = models.ForeignKey(
        Status, on_delete=models.CASCADE, related_name="status_data_name")
    week = models.CharField(max_length=25, default=None)
    create_date = models.DateField(auto_now_add=True)

    def __str__(self):
        return "%s %s %s" % (self.kpi_data_name.site_lang_coun_matrix.site_lang_matrix_name.lang_name, self.create_date, self.status_name.name, self.week)


class KpiInput(models.Model):
    days = [('MONDAY', 'Monday'), ('TUESDAY', 'Tuesday'), ('WEDNESDAY', 'Wednesday'), ('THURSDAY', 'Thursday'), ('FRIDAY', 'Friday'), ('SATURDAY', 'Saturday'), ('SUNDAY', 'Sunday')]
    sitelanguagecountry_matrix = models.ForeignKey(SiteLanguageCountryMatrix, on_delete=models.CASCADE, related_name='sitelanguecountry_matrix')
    status = models.ForeignKey(Status, on_delete=models.CASCADE, related_name='status')
    day = models.CharField(choices=days, max_length=9, default='MONDAY')
    year = models.DateField(auto_now_add=True)

    def __str__(self):
        return f'{self.sitelanguagecountry_matrix.site_master.site_name, self.status, self.day, self.year}'
